...
namespace Whale.API.Controllers.Slack
{
    public class SlackController : ControllerBase
    {
        ...
        public async Task StartMeeting(SlackCommand userData)
        {
            ...
            if (userEmail == null) await _slackService.SendSlackReplyAsync("You should to specify email. (ex. /whale user@gmail.com)", userData.response_url);
            ...
            try
            {
                ...
                await _slackService.SendSlackReplyAsync(link, userData.response_url);
            }
            catch(Exception)
            {
                await _slackService.SendSlackReplyAsync("Some troubles happened", userData.response_url);
            }
        }
    }
}
