...
export class SettingAudioComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.browserMediaDevice.getAudioInputList()
    .then(res => this.inputDevices = res)
    .catch(error => console.log(error));
    this.browserMediaDevice.getAudioOutputList()
  ...
  // tslint:disable-next-line: typedef
  onChange(){
    if (this.inputDevice === undefined) { this.inputDevice = this.inputDevices[0]; }
    console.log(this.inputDevices);
    this.meter.connect(this.inputDevice).catch(err => alert('Connection Error'));
