...
export class UserRegistrationService {
  ...
  registerUser(currentUser: any) {
    ...
    this.http.get<User>(this.getUserUrl + `/${user.email}`, {observe: 'response'}).subscribe(
      user => console.log(`User ${user.body.email} exists`),
      error => {
        if (error.status === 404) {
          this.httpService.postClearRequest<IFireBaseUser, User>(this.addUserUrl, user).subscribe();
        }
    })
  }
}
