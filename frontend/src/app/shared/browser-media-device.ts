export class BrowserMediaDevice {
  ...
  private getMediaDevicesByKind(kind: string): Promise<MediaDeviceInfo[]> {
    return this.getDeviceListPromise().then((devices) => {
  ...
  private createMediaTrackConstraint(
  ...
  getDeviceConstraints(
    ...
    const audioConstraint = this.createMediaTrackConstraint(
      this.getCurrentAudioInput()
    );
    const videoConstraint = this.createMediaTrackConstraint(
      this.getCurrentVideoInput()
    );
    ...
    return Object.assign({}, initialConstraints, {
      audio: this.isConstraintDisabled(initialConstraints, 'audio')
        ? false
        : audioConstraint,
      video: this.isConstraintDisabled(initialConstraints, 'video')
        ? false
        : videoConstraint,
    });
  }
